...
public:
  ...
  void detect(const cv::Mat &bgrImage, const cv::Mat &depth, const pcl::PointCloud<pcl::PointXYZ> &sceneCloud, std::vector<PoseRT> &poses_cam, std::vector<float> &posesQualities, std::vector<std::string> &objectNames) const;
